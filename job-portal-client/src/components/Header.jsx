      ...
      <div className='bg-[#124429]'>
        ...
        >
          <div className='w-full z-10'>
            <div className='mb-8'>
              <p className='text-white font-bold text-5xl mt-24 '>{title}</p>
            </div>
  
            <div className='w-full mt-24 flex items-center justify-around bg-white px-2 md:px-5 py-2.5 md:py-6 shadow-2xl rounded-full'>
              <SearchInput
                placeholder='Job Title or Keywords'
                icon={<AiOutlineSearch className='text-gray-900 text-xl' />}
                value={searchQuery}
                setValue={setSearchQuery}
              />
              <SearchInput
                placeholder='Add Country or City'
                icon={<CiLocationOn className='text-gray-900 text-xl' />}
                value={location}
            ...
            {type && (
              <div className='w-full lg:1/2 flex flex-wrap gap-3 md:gap-6 py-10 md:py-14'>
                {popularSearch.map((search, index) => (
                  <span
                    key={index}
                    className='bg-[#b3f2bf] text-green-900 py-1 px-2 rounded-full text-sm md:text-base'
                  >
                    {search}
                  </span>
                ))}
          ...
          <div className='w-1/3 h-full mt-10 absolute top-24 md:-top-6 lg:-top-14 right-16 2xl:right-[18rem]'>
